# Deployed: 2025-11-14 03:03:10 UTC
...
def filter_dataframe_by_version(df: DataFrame, existing_versions: Set[str]) -> DataFrame:
    ...
    window_spec = Window.partitionBy("medication_id").orderBy(F.col("meta_last_updated").desc())
    ...
    if incoming_count > deduplicated_count:
        logger.info(f"Deduplicated incoming data: {incoming_count} → {deduplicated_count} records (kept latest per medication)")
...
def main():
    ...
    try:
        ...
        table_names = ["medication_identifiers", "medications"]
