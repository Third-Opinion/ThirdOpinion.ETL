...
def truncate_extensions_if_needed(extensions_field, max_length=65535):
    """Truncate extensions field if it exceeds Redshift VARCHAR(MAX) limit"""
    if extensions_field is None:
        return None
    
    extensions_str = str(extensions_field)
    if len(extensions_str) > max_length:
        logger.warning(f"⚠️ Extensions field too long ({len(extensions_str)} chars), truncating to {max_length}")
        return extensions_str[:max_length-3] + "..."
    
    return extensions_str

# Define UDFs globally so they can be used in all transformation functions
convert_to_json_udf = F.udf(convert_to_json_string, StringType())
truncate_extensions_udf = F.udf(truncate_extensions_if_needed, StringType())
...
def transform_main_condition_data(df):
    ...
    select_columns.extend([
        F.to_timestamp(F.col("onsetDateTime"), "yyyy-MM-dd'T'HH:mm:ssXXX").alias("onset_datetime"),
    ...
    select_columns.extend([
        F.to_timestamp(F.col("abatementDateTime"), "yyyy-MM-dd'T'HH:mm:ssXXX").alias("abatement_datetime"),
    ...
    select_columns.extend([
        F.to_timestamp(F.col("recordedDate"), "yyyy-MM-dd'T'HH:mm:ssXXX").alias("recorded_date"),
    ...
    # Add extensions (with truncation to prevent Redshift VARCHAR length issues)
    select_columns.extend([
        truncate_extensions_udf(convert_to_json_udf(F.col("extension"))).alias("extensions"),
...
def main():
    ...
    try:
        ...
        expected_columns = {
            ...
            "meta_security", "meta_tag", "extensions", "created_at", "updated_at"
        }
        ...
        logger.info(f"Final counts - Categories: {categories_final_count}, Notes: {notes_final_count}, Body Sites: {body_sites_final_count}, Stages: {stages_final_count}, Codes: {codes_final_count}, Evidence: {evidence_final_count}")
        ...
        # Check extensions field length to prevent Redshift VARCHAR length issues
        logger.info("🔍 Checking extensions field lengths...")
        extensions_lengths = main_final_df.select(
            F.length(F.col("extensions")).alias("ext_length")
        ).filter(F.col("ext_length").isNotNull()).collect()
        
        if extensions_lengths:
            max_length = max([row.ext_length for row in extensions_lengths])
            logger.info(f"📏 Maximum extensions field length: {max_length} characters")
            if max_length > 65535:
                logger.warning(f"⚠️ Some extensions fields exceed Redshift VARCHAR(MAX) limit (65535 chars)")
                logger.warning("The truncation function should handle this, but check the data quality.")
            else:
                logger.info("✅ All extensions fields are within Redshift VARCHAR(MAX) limit")
        ...
        total_output_records = main_count + categories_count + notes_count + body_sites_count + stages_count + codes_count + evidence_count
