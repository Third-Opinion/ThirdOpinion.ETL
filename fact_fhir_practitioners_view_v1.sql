-- ===================================================================
-- FACT FHIR PRACTITIONERS VIEW V1
-- ===================================================================
-- 
-- OVERVIEW:
-- Comprehensive materialized view that creates a fact table from FHIR Practitioner resources
-- by joining the main practitioners table with practitioner names, addresses, and telecoms for analytics and reporting.
-- Adapts the name ranking logic from the patient view for consistency.
-- 
-- PRIMARY KEY: practitioner_id
-- 
-- SOURCE TABLES:
-- - public.practitioners: Core practitioner data including active status
-- - public.practitioner_names: Practitioner name information
-- - public.practitioner_addresses: Practitioner addresses and locations
-- - public.practitioner_telecoms: Practitioner contact information (phone, email, etc.)
-- 
-- REFRESH STRATEGY:
...
-- 
-- DATA PROCESSING:
-- - Uses same name ranking logic as patient view for consistency
-- - Aggregates multiple addresses and contact methods as JSON
...
-- Names are ranked by preference (adapted from patient view):
-- 1. Names with both family and given names (rank 1)
-- 2. Names with only family name (rank 2)
-- 3. Names with only given name (rank 3)
-- Within each rank, ordered by family, given names alphabetically
-- 
-- FILTERING:
-- - Includes all practitioners regardless of active status
-- - Uses LEFT JOINs to preserve practitioners without related data
-- 
-- OUTPUT COLUMNS:
-- - All core practitioner fields from practitioners table
-- - names: JSON object containing primary name information
-- - addresses: JSON array of practitioner addresses
-- - telecoms: JSON array of contact methods
...
-- - Groups by all practitioner fields to aggregate related data
...
-- - Practitioner directory and contact management
-- - Provider network analysis and reporting
-- - Clinical workflow and assignment systems
...
SELECT 
    ...
    -- FHIR Metadata
    pr.meta_version_id,              -- FHIR resource version
