...
def transform_unique_codes(
    ...
    """
    Extract unique codes from condition codes DataFrame
    Creates codes table data with deterministic code_id
    
    Args:
        codes_df: DataFrame with code_code, code_system, code_display, code_text
        processing_config: Processing configuration
        
    Returns:
        DataFrame with unique codes: code_id, code_code, code_system, code_display, 
        code_text (truncated to 1000)
    """
    ...
    # Truncate code_text to 1000 chars
    ...
    unique_codes = codes_cleaned.select(
        ...
        F.lit(None).cast("string").alias("normalized_code_text")  # Conditions don't use normalized_code_text
...
def transform_condition_codes_with_code_id(
    codes_df: DataFrame,
    processing_config: Optional[ProcessingConfig] = None,
    glue_context: Optional[GlueContext] = None,
    config: Optional[DatabaseConfig] = None,
    ...
    """
    Transform condition codes to use code_id references
    
    Args:
        codes_df: DataFrame with condition_id, patient_id, code_code, code_system, 
                  code_display, code_text
        processing_config: Processing configuration
        
    Returns:
        DataFrame with condition_id, patient_id, code_id, code_rank
    """
    logger.info("Transforming condition codes to use code_id references...")
    ...
    if "code_id" in codes_df.columns:
        logger.info("code_id already exists in codes_df - reusing (optimization)")
        condition_codes_normalized = codes_df.filter(
            F.col("code_id").isNotNull() & 
        ...
        if existing_codes_df is not None and not existing_codes_df.rdd.isEmpty():
            # Step 2: Broadcast existing code_ids for efficient join
            logger.info("Broadcasting existing code_ids for reuse in condition_codes...")
            existing_codes_broadcast = F.broadcast(existing_codes_df)
            ...
            # Step 4: Generate new code_ids only for codes that don't exist (using native Spark)
            condition_codes_normalized = codes_with_existing_ids.select(
                F.col("codes.*"),  # All columns from codes_df
                F.col("existing.code_id").alias("existing_code_id")  # Existing code_id from Redshift
            ).withColumn(
                "code_id",
            ...
            # No existing codes or initial load - generate all code_ids using native Spark
            logger.info("Generating all code_ids using native Spark MD5 (no UDF)...")
            condition_codes_normalized = codes_df.withColumn(
                "code_id",
                generate_code_id_native(
                    F.col("code_code"),
                    F.col("code_system")
    ...
    if "array_position" in condition_codes_normalized.columns:
        condition_codes_final = condition_codes_normalized.select(
            F.col("condition_id"),
            F.col("patient_id"),
            F.col("code_id"),
    ...
    # CRITICAL: Deduplicate codes by (condition_id, code_id) to prevent duplicate codes
    logger.info("Deduplicating codes by (condition_id, code_id) to prevent duplicates...")
    condition_codes_final = condition_codes_final.dropDuplicates(["condition_id", "code_id"])
    logger.info("✅ Deduplicated codes (removed duplicate code_id per condition)")
    
    logger.info("✅ Transformed condition-code relationships (count skipped to avoid shuffle failures)")
    
    return condition_codes_final
