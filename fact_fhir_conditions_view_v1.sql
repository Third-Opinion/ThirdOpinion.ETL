...
AUTO REFRESH NO
AS
WITH aggregated_body_sites AS (
    SELECT 
        cb.condition_id,
        JSON_PARSE(
            '[' || LISTAGG(DISTINCT
                '{' ||
                '"system":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(cb.body_site_system, '[\r\n\t]', ''), '"', ''), '') || '",' ||
                '"code":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(cb.body_site_code, '[\r\n\t]', ''), '"', ''), '') || '",' ||
                '"display":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(cb.body_site_display, '[\r\n\t]', ''), '"', ''), '') || '",' ||
                '"text":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(cb.body_site_text, '[\r\n\t]', ''), '"', ''), '') || '"' ||
                '}',
                ','
            ) WITHIN GROUP (ORDER BY cb.body_site_system, cb.body_site_code) || ']'
        ) AS body_sites
    FROM public.condition_body_sites cb
    WHERE cb.body_site_code IS NOT NULL
    GROUP BY cb.condition_id
),
aggregated_categories AS (
    SELECT 
        ...
        JSON_PARSE(
            '[' || LISTAGG(DISTINCT
                '{' ||
                '"system":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(cat.category_system, '[\r\n\t]', ''), '"', ''), '') || '",' ||
                '"code":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(cat.category_code, '[\r\n\t]', ''), '"', ''), '') || '",' ||
                '"display":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(cat.category_display, '[\r\n\t]', ''), '"', ''), '') || '",' ||
                '"text":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(cat.category_text, '[\r\n\t]', ''), '"', ''), '') || '"' ||
                '}',
                ','
            ) WITHIN GROUP (ORDER BY cat.category_system, cat.category_code) || ']'
        ) AS categories
    FROM public.condition_categories cat
    WHERE cat.category_code IS NOT NULL
    GROUP BY cat.condition_id
),
aggregated_evidence AS (
    SELECT 
        ...
        JSON_PARSE(
            '[' || LISTAGG(DISTINCT
                '{' ||
                '"system":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(ce.evidence_system, '[\r\n\t]', ''), '"', ''), '') || '",' ||
                '"code":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(ce.evidence_code, '[\r\n\t]', ''), '"', ''), '') || '",' ||
                '"display":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(ce.evidence_display, '[\r\n\t]', ''), '"', ''), '') || '",' ||
                '"detail_reference":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(ce.evidence_detail_reference, '[\r\n\t]', ''), '"', ''), '') || '"' ||
                '}',
                ','
            ) WITHIN GROUP (ORDER BY ce.evidence_system, ce.evidence_code) || ']'
        ) AS evidence
    FROM public.condition_evidence ce
    WHERE ce.evidence_code IS NOT NULL
...
aggregated_notes AS (
    SELECT 
        ...
        JSON_PARSE(
            '[' || LISTAGG(
                '{' ||
                '"text":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(cn.note_text, '[\r\n\t]', ''), '"', ''), '[\\\\]', '\\\\\\\\'), '') || '",' ||
                '"author_reference":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(cn.note_author_reference, '[\r\n\t]', ''), '"', ''), '') || '",' ||
                '"time":"' || COALESCE(cn.note_time::VARCHAR, '') || '"' ||
                '}',
                ','
            ) WITHIN GROUP (ORDER BY cn.note_time) || ']'
        ) AS notes
    FROM public.condition_notes cn
    WHERE cn.note_text IS NOT NULL
    GROUP BY cn.condition_id
),
aggregated_stages AS (
    SELECT 
        ...
        JSON_PARSE(
            '{' ||
            '"summary":{' ||
                '"system":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(MAX(cs.stage_summary_system), '[\r\n\t]', ''), '"', ''), '') || '",' ||
                '"code":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(MAX(cs.stage_summary_code), '[\r\n\t]', ''), '"', ''), '') || '",' ||
                '"display":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(MAX(cs.stage_summary_display), '[\r\n\t]', ''), '"', ''), '') || '"' ||
            '},' ||
            '"assessment":{' ||
                '"system":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(MAX(cs.stage_assessment_system), '[\r\n\t]', ''), '"', ''), '') || '",' ||
                '"code":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(MAX(cs.stage_assessment_code), '[\r\n\t]', ''), '"', ''), '') || '",' ||
                '"display":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(MAX(cs.stage_assessment_display), '[\r\n\t]', ''), '"', ''), '') || '"' ||
            '},' ||
            '"type":{' ||
                '"system":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(MAX(cs.stage_type_system), '[\r\n\t]', ''), '"', ''), '') || '",' ||
                '"code":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(MAX(cs.stage_type_code), '[\r\n\t]', ''), '"', ''), '') || '",' ||
                '"display":"' || COALESCE(REGEXP_REPLACE(REGEXP_REPLACE(MAX(cs.stage_type_display), '[\r\n\t]', ''), '"', ''), '') || '"' ||
            '}' ||
            '}'
        ) AS stages
    FROM public.condition_stages cs
    GROUP BY cs.condition_id
)
SELECT 
    -- ============================================
    -- CORE CONDITION DATA
    -- ============================================
    c.condition_id,                      -- Unique condition identifier (Primary Key)
    c.patient_id,                        -- Patient reference
    c.encounter_id,                      -- Associated encounter (if any)
    
    -- Clinical Status Fields
    c.clinical_status_code,              -- active | inactive | resolved
    c.clinical_status_display,
    c.clinical_status_system,
    
    -- Verification Status Fields
    c.verification_status_code,          -- confirmed | provisional | differential | refuted
    c.verification_status_display,
    c.verification_status_system,
    
    -- Condition Description
    c.condition_text,                    -- Human-readable condition description
    ...
    ROW_NUMBER() OVER (PARTITION BY c.condition_id ORDER BY 
        CASE cc.code_system
            WHEN 'http://snomed.info/sct' THEN 1
            WHEN 'http://hl7.org/fhir/sid/icd-10-cm' THEN 2
            WHEN 'http://hl7.org/fhir/sid/icd-10' THEN 3
            ELSE 4
        END,
        cc.code_code
    ) AS code_rank,
    ...
    CASE 
        WHEN c.onset_datetime IS NOT NULL AND c.abatement_datetime IS NOT NULL 
        THEN DATEDIFF(day, c.onset_datetime, c.abatement_datetime)
        WHEN c.onset_period_start IS NOT NULL AND c.abatement_period_start IS NOT NULL
        THEN DATEDIFF(day, c.onset_period_start, c.abatement_period_start)
        ELSE NULL 
    END AS condition_duration_days,
    
    -- Determine if condition is currently active
    CASE 
        WHEN c.clinical_status_code = 'active' AND (c.abatement_datetime IS NULL OR c.abatement_datetime > CURRENT_DATE)
        THEN TRUE
        ELSE FALSE
    END AS is_active
...
-- - Consider SORTKEY on (patient_id, code_system, code_code) for code-based analysis
-- - Consider SORTKEY on (condition_id, code_rank) for primary code queries
-- ===================================================================

-- ===================================================================
-- DATA QUALITY NOTES
-- ===================================================================
-- 1. DENORMALIZED STRUCTURE: Each condition may produce multiple rows (one per code)
-- 2. Conditions without codes will still appear with NULL code columns (LEFT JOIN preserved)
-- 3. Historical conditions may lack abatement information
-- 4. Some conditions may not have associated encounters
-- 5. Extension data aggregation excluded due to complexity - query directly if needed
-- 6. All text fields are sanitized to ensure valid JSON format
-- 7. Code ranking prioritizes: SNOMED (1) > ICD-10-CM (2) > ICD-10 (3) > Others (4)
-- 8. Use code_rank = 1 to filter for primary diagnostic codes only
-- ===================================================================

-- ===================================================================
-- DENORMALIZATION IMPLEMENTATION NOTES
-- ===================================================================
-- This view uses denormalization strategy for condition codes:
-- - Original conditions table: 3.2M records
-- - Denormalized view: ~5.9M records (one per condition-code combination)
-- - Benefits: Direct access to code columns without JSON parsing
-- - Trade-offs: Larger result set, repeated condition data across code rows
-- - Use code_rank column to identify primary vs secondary codes
-- - Query patterns: Filter by code_system/code_code for prevalence analysis
-- ===================================================================