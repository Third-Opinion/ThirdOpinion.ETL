CREATE MATERIALIZED VIEW rpt_fhir_observations_ast_hmu_v1
DISTSTYLE KEY
DISTKEY (patient_id)
...
WHERE tp.observation_category = 'laboratory'
    ...
    AND (
        tp.observation_text ILIKE '%AST%'
        OR tp.observation_text ILIKE '%aspartate%aminotransferase%'
        OR tp.observation_text ILIKE '%SGOT%'
    )
