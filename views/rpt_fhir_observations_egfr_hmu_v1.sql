CREATE MATERIALIZED VIEW rpt_fhir_observations_egfr_hmu_v1
DISTSTYLE KEY
DISTKEY (patient_id)
...
WHERE tp.observation_category = 'laboratory'
    ...
    AND (
        tp.observation_text ILIKE '%egfr%'
        OR tp.observation_text ILIKE '%GFR%'
        OR tp.observation_text ILIKE '%creatinine%clearance%'
    )
