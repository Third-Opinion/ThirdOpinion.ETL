CREATE MATERIALIZED VIEW rpt_fhir_observations_hba1c_hmu_v1
DISTSTYLE KEY
DISTKEY (patient_id)
...
WHERE tp.observation_category = 'laboratory'
    ...
    AND (
        tp.observation_text ILIKE '%HbA1c%'
        OR tp.observation_text ILIKE '%hemoglobin A1c%'
        OR tp.observation_text ILIKE '%glycohemo%'
        OR tp.observation_text ILIKE '%glycosylated%'
        OR tp.observation_text ILIKE '%A1C%'
    )
