CREATE MATERIALIZED VIEW rpt_fhir_observations_serum_albumin_hmu_v1
DISTSTYLE KEY
DISTKEY (patient_id)
...
WHERE tp.observation_category = 'laboratory'
    ...
    AND (
        tp.observation_text ILIKE '%albumin%'
        OR (tp.observation_text ILIKE '%ALB%' AND tp.observation_text NOT ILIKE '%globulin%')
    )
